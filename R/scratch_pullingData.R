...
temp.values <- temp.data$Value[59:134]

#limit temp data to just the years I am working with 



#change conditionals from above - bring down to here 
log.like.calc <- function(data, location1, scale1, shape1){
  ll<-sum(devd(data, loc=location1, scale=scale1, shape=shape1, type=c('GEV'), log=TRUE))
  return(ll)
}
...
#----non stationary mu only 
p.names <- c('mu0', 'mu1', 'sigma', 'xi')

upper.bound <- c(3000,100, 3000, 5)
lower.bound <- c(0,0, 0, -5)

optim.like.temp <- DEoptim(neg.log.like.calc, lower=lower.bound, upper=upper.bound, control=de.optim.val, data = lsl.max, temps = temp.values)
#temp.val.to.use <- pevd(x.hgt, loc = optim.like.temp$optim$bestmem[1], scale=optim.like.temp$optim$bestmem[3], shape=optim.like.temp$optim$bestmem[4], type=c('GEV'), lower.tail = FALSE)

plot(x.hgt, log10(sf.hgt), type='l', ylab = 'log probability', xlab = 'height [mm] of sea')
title('Survival Function of height of sea [mm], New London CT')
points(lsl.sorted.vals, log10(esf.vals))
for (j in 1:length(temp.values)){
  sf.optim2 <- 1 - pevd(lsl.max[j], loc = (optim.like.temp$optim$bestmem[1] + optim.like.temp$optim$bestmem[2] * temp.values[j]), scale = optim.like.temp$optim$bestmem[3] , shape = optim.like.temp$optim$bestmem[4],type=c('GEV'))
  points(lsl.max[j], log10(sf.optim2), pch = 2, col='red')
  
}
#-----non sationary mu and sigma --------------------------------------------------- --------------------------------------------------- --------------------------------------------------- 
...
optim.like.temp2 <- DEoptim(neg.log.like.calc, lower=lower.bound, upper=upper.bound, control=de.optim.val, data = lsl.max, temps = temp.values)
...
points(lsl.sorted.vals, log10(esf.vals))
for (j in 1:length(temp.values)){
    sf.optim3 <- 1 - pevd(lsl.max[j], loc = (optim.like.temp2$optim$bestmem[1] + optim.like.temp2$optim$bestmem[2] * temp.values[j]), scale = exp(optim.like.temp2$optim$bestmem[3] + optim.like.temp2$optim$bestmem[4]*temp.values[j]), shape = optim.like.temp2$optim$bestmem[5],type=c('GEV'))
    points(lsl.max[j], log10(sf.optim3), pch = 2, col='green')
  
}
#------all variables non sationary --------------------------------------------------- --------------------------------------------------- --------------------------------------------------- 
p.names <- c('mu0', 'mu1', 'sigma0', 'sigma1' , 'xi0', 'xi1')

upper.bound <- c(3000,100, 1000, 10 , 1, 1)
lower.bound <- c(0,-100, 0, 0, -1, -1)

optim.like.temp3 <- DEoptim(neg.log.like.calc, lower=lower.bound, upper=upper.bound, control=de.optim.val, data = lsl.max, temps = temp.values)
...
#--------------------------RMSE For Optim Fits , I'm confused with what I'm supposed to use here though
...
rmse.sigma<- sqrt(1/(length(lsl.max))*(sum(lsl.max[i] - sf.optim2[i], i = 1, length(lsl.max)))**2)

#xi non stationary 
rmse.xi <- sqrt(1/(length(lsl.max))*(sum(lsl.max[i] - sf.optim2[i], i = 1, length(lsl.max)))**2)

#mu and sigma non stationary 
rmse.mu.sigma <- sqrt(1/(length(lsl.max))*(sum(lsl.max[i] - sf.optim2[i], i = 1, length(lsl.max)))**2)

#mu and xi non stationary 
rmse.mu.xi<- sqrt(1/(length(lsl.max))*(sum(lsl.max[i] - sf.optim2[i], i = 1, length(lsl.max)))**2)

#sigma and xi non stationary 
rmse.sigma.xi <- sqrt(1/(length(lsl.max))*(sum(lsl.max[i] - sf.optim2[i], i = 1, length(lsl.max)))**2)

#mu, sigma, xi non stationary 
rmse.mu.sigma.xi <- sqrt(1/(length(lsl.max))*(sum(lsl.max[i] - sf.optim2[i], i = 1, length(lsl.max)))**2)
...
aic.mu.sigma <- 2*2 - 2*log()

aic.mu.xi<- 2*2 - 2*log()

aic.sigma.xi<- 2*2 - 2*log()

aic.mu.sigma.xi <- 2*3 - 2*log()

#------------------BIC For Optim Fits 
#kln(n) - 2 ln(L)

bic.mu<- 1*log(length(lsl.max)) - 2*log()

bic.sigma <- 1*log(length(lsl.max))- 2*log()

bic.xi <- 1*log(length(lsl.max))- 2*log()

bic.mu.sigma<- 2*log(length(lsl.max))- 2*log()

bic.sigma.xi <- 2*log(length(lsl.max))- 2*log()

bic.mu.sigma.xi <- 3*log(length(lsl.max))- 2*log()
